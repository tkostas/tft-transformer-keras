...
model_init_args = parser.add_argument_group(
    title="Model definition params", description="Define model specific parameters."
)
...
training_args = parser.add_argument_group(
    title="Model training arguments", description="Define training configuration."
)
...
training_args.add_argument(
    "--epochs", default=3, help="Number of epochs to train the model.", type=int
)
training_args.add_argument("--batch_sz", default=32, help="Batch size.", type=int)
training_args.add_argument("--lr", default=0.001, help="Learning rate.", type=float)
training_args.add_argument(
    "--clip_norm",
    default=None,
...
training_args.add_argument(
    "--dropout_rate", default=0.1, help="Dropout rate to apply.", type=float
)
training_args.add_argument(
    "--l2_reg", default=None, help="L2 regularization value", type=float
)
training_args.add_argument(
    "--optimizer",
    default="sgd",
...
eval_args = parser.add_argument_group(
    title="Model evaluation arguments", description="Define model evaluation process"
)
...
callback_args = parser.add_argument_group(
    title="Callback arguments", description="Define callback configuration"
)

callback_args.add_argument(
    "--log_dir", default=None, help="Log dir for TensorBoard callback."
)
...
def main():
    ...
    dset = datasets_factory[dataset_name](
        ts_len=config.ts_len, n_enc_steps=config.n_enc_steps, sample_sz=config.sample_sz
    )

    (x_train, y_train), (x_val, y_val), (x_test, y_test) = dset.extract_xy_pairs(
        ts_len=config.ts_len, n_dec_steps=config.n_dec_steps
    )
    ...
    quick_evaluation(model, x_test, x_train, x_val, y_test, y_train, y_val, config)
